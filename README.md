# ESP32-C3 Super Mini + Touch Display
...
This file contains the WiFi credentials for the ESP32-C3. It should be created manually and should not be committed to version control. See secrets.h.example for an example.
...
This file contains the pixel data of the splash screen image, which is displayed on startup. It is generated by the `convert_image.py` script.

## Helper scripts

### `setup.sh`

This script sets up the ESP32-C3 Touch project using the Arduino CLI.

### `compile.sh`

This script compiles the ESP32-C3 Touch project using the Arduino CLI.

### `upload.sh`

This script uploads the compiled firmware to the ESP32-C3 using the Arduino CLI.

### `monitor.sh`

This script monitors the serial output of the ESP32-C3 using the Arduino CLI.